...
class ObjectDetectionEvaluator(DetectionEvaluator):
  ...
  def __init__(self,
               ...
               matching_iou_threshold_2 = 0.75, #added
    ...
    self._matching_iou_threshold_2 = matching_iou_threshold_2 #added
    ...
    #added
    self._evaluation_2 = ObjectDetectionEvaluation(
        num_groundtruth_classes=self._num_classes,
        matching_iou_threshold=self._matching_iou_threshold_2,
        use_weighted_mean_ap=self._use_weighted_mean_ap,
        label_id_offset=self._label_id_offset,
        group_of_weight=self._group_of_weight)
    self._image_ids = set([])
  ...
  def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
    ...
    self._evaluation.add_single_ground_truth_image_info(
        ...
        #added
    self._evaluation_2.add_single_ground_truth_image_info(
        image_key=image_id,
        groundtruth_boxes=groundtruth_dict[
            standard_fields.InputDataFields.groundtruth_boxes],
        groundtruth_class_labels=groundtruth_classes,
  ...
  def add_single_detected_image_info(self, image_id, detections_dict):
    ...
    #added
    self._evaluation_2.add_single_detected_image_info(
        image_key=image_id,
        detected_boxes=detections_dict[
            standard_fields.DetectionResultFields.detection_boxes],
        detected_scores=detections_dict[
  ...
  def evaluate(self):
    ...
    """
    (per_class_ap, mean_ap, _, _, per_class_corloc, mean_corloc, per_class_f1, mean_f1) = (
    ...
    pascal_metrics[self._metric_prefix + 'F1/meanF1score@{}IOU'.format(
        self._matching_iou_threshold)] = mean_f1
    ...
    for idx in range(per_class_ap.size):
      if idx + self._label_id_offset in category_index:
        ...
        display_name = (
            self._metric_prefix + 'F1/PerformanceByCategory/F1@{}IOU/{}'.format(
                self._matching_iou_threshold,
                category_index[idx + self._label_id_offset]['name']))
        pascal_metrics[display_name] = per_class_f1[idx]
    ...
    ### added
    self._evaluation.clear_detections()
    (per_class_ap_2, mean_ap_2, _, _, per_class_corloc_2, mean_corloc_2, per_class_f1_2, mean_f1_2) = (
        self._evaluation_2.evaluate())
    pascal_metrics[
        self._metric_prefix +
        'Precision/mAP@{}IOU'.format(self._matching_iou_threshold_2)]=mean_ap_2

  
    pascal_metrics[self._metric_prefix + 'F1/meanF1score@{}IOU'.format(
        self._matching_iou_threshold_2)] = mean_f1_2

    if self._evaluate_corlocs:
      pascal_metrics[self._metric_prefix + 'Precision/meanCorLoc@{}IOU'.format(
          self._matching_iou_threshold_2)] = mean_corloc_2
    category_index = label_map_util.create_category_index(self._categories)
    for idx in range(per_class_ap.size):
      if idx + self._label_id_offset in category_index:
        display_name = (
            self._metric_prefix + 'PerformanceByCategory/AP@{}IOU/{}'.format(
                self._matching_iou_threshold_2,
                category_index[idx + self._label_id_offset]['name']))
        pascal_metrics[display_name] = per_class_ap_2[idx]

        display_name = (
            self._metric_prefix + 'F1/PerformanceByCategory/F1@{}IOU/{}'.format(
                self._matching_iou_threshold_2,
                category_index[idx + self._label_id_offset]['name']))
        pascal_metrics[display_name] = per_class_f1_2[idx]
        ...
        if self._evaluate_corlocs:
          display_name = (
              self._metric_prefix + 'PerformanceByCategory/CorLoc@{}IOU/{}'
              .format(self._matching_iou_threshold_2,
                      category_index[idx + self._label_id_offset]['name']))
          pascal_metrics[display_name] = per_class_corloc_2[idx]
...
########################## Change Made ########################### #added
  def __init__(self,
               categories,
               matching_iou_threshold=0.5,
               matching_iou_threshold_2=0.75,
               evaluate_corlocs=True, #added False
...
ObjectDetectionEvalMetrics = collections.namedtuple(
    'ObjectDetectionEvalMetrics', [
        ...
        'mean_corloc', 'average_f1s', 'mean_f1'
    ])
...
class ObjectDetectionEvaluation(object):
  ...
  def _initialize_detections(self):
    ...
    self.average_f1_per_class = np.empty(self.num_class, dtype=float)
    self.average_f1_per_class.fill(np.nan)
  ...
  def evaluate(self):
    ...
    for class_index in range(self.num_class):
      ...
      f1_score = metrics.compute_mean_f1_score(precision, recall)
      self.average_f1_per_class[class_index] = f1_score
    ...
    if self.use_weighted_mean_ap:
      ...
      mean_f1 = np.nanmean(self.average_f1_per_class)
    mean_corloc = np.nanmean(self.corloc_per_class)
    return ObjectDetectionEvalMetrics(
        self.average_precision_per_class, mean_ap, self.precisions_per_class,
        self.recalls_per_class, self.corloc_per_class, mean_corloc, self.average_f1_per_class, mean_f1)
